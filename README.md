...
**Huom!** Tällä hetkellä ohjelma oletusarvoisesti suorittaa käyttäjänimien mäppäyksen. Taustalla on asiakastapauksesta johtuva priorisointi, jossa _EI_ voida käyttää asiakkaan GitLabissa olevia sähköposteja. Projektin backlogilla on lisättynä [feature](https://github.com/EficodeRjpalt/ot-harjoitustyo/issues/63), jossa mahdollistetaan asetus, jolla voidaan määritellä että kyätetäänkö käyttäjien mäppäystä ja sähköpostin "arvausta" vai haetaanko suoraan käyttäjän sähköposti GitLabista.
